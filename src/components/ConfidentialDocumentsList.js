...
const ConfidentialDocumentsList = () => {
  const { keycloak } = useKeycloak();
  const [documents, setDocuments] = useState(() => {
    const savedDocuments = localStorage.getItem('confidentialDocuments');
  ...
  useEffect(() => {
    localStorage.setItem('confidentialDocuments', JSON.stringify(documents));
  }, [documents]);
...
export default ConfidentialDocumentsList;